...
public extension String {
    ...
    func indexOf(substring: String, fromIndex: Int) -> Int? {
        let newStr = self.substring(fromIndex, length: self.length - fromIndex)
        if let range = newStr.rangeOfString(substring) {
            return startIndex.distanceTo(range.startIndex) + fromIndex
        }
        return nil
    }
